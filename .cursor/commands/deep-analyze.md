...
**Objective**: You are an AI-driven workflow analysis specialist. Your task is to perform deep-dive assessments of five core AI-workflow directories while honoring session suggestion history, so that every session advances a unified AI-driven workflow blueprint with evidence-backed findings.

**Primary Mission**: Deliver a comprehensive technical analysis for each target directory:
...
**Critical Principles**:
1. Only add suggestions if genuine gaps exist. Do not add suggestions for the sake of completeness if no real gaps are found.
2. Start every round from the cumulative approved guidance so the AI-driven workflow blueprint continuously improves instead of resetting each session.
3. Treat continuity tasks (approved suggestions) as supporting context—the session’s core value is the new analysis of the five target directories.
...
### Step 2.3: Build Approved Suggestion Register

**[STRICT] Action**: Organize every approved suggestion so it is reused accurately in the new session output.

**Register Structure (per workflow)**:
1. **Carried Forward Bucket** – Suggestions with status NOT IMPLEMENTED, PARTIALLY IMPLEMENTED, or CANNOT VERIFY. These remain active tasks.
2. **Implemented Highlights Bucket** – Suggestions marked IMPLEMENTED. These must be cited as completed wins when drafting strengths or integration callouts.

**Formatting Requirements**:
- Preserve exact wording from the original suggestion for both buckets.
- Prefix each entry with source session: `[Session XX | Status: <status>]`
- Include original priority, file path, and evidence references.
- Store buckets in a structure that can be injected directly into the session file template (see Phase 4).

**Expected Result**: Comprehensive register covering both unresolved and implemented approved suggestions.

---

### Step 2.4: Synthesize Integrated Baseline

**[STRICT] Action**: Use the Approved Suggestion Register to seed the new session narrative before hunting for fresh gaps.

**Procedure**:
1. Merge Carried Forward Buckets into the "Carried Forward Suggestions" section for each workflow (Phase 4 template).
2. Translate Implemented Highlights into succinct bullet points for the "Strengths" section or dedicated "Implemented Improvements" notes.
3. Confirm no approved suggestion is lost: every approved item must appear in the new session as either an ongoing action or a documented win.
4. Once continuity is locked in, dedicate the majority of session time to fresh analysis across all five target directories.

**Expected Result**: Baseline content ready so new analysis only adds genuinely new insights on top of accepted history.

---

## Phase 3: Workflow Directory Analysis (Core Workload)

### Step 3.1: Verify Directory Paths

**[STRICT] Action**: Verify all five target directories exist and are accessible.
...
**[STRICT] Action**: Perform systematic analysis of each accessible workflow directory. This phase is the primary deliverable—allocate most session time here.
...
```
Structure Analysis:
- Target path: [absolute directory path]
...
- Key takeaways: [1-2 sentences summarizing structural impact on AI workflow]
```
...
- Key takeaways: [1-2 sentences connecting code quality to workflow reliability]
```
...
- Key takeaways: [note how docs support or block the AI workflow]
```
...
- Key takeaways: [describe readiness for integration/automation]
```
...
- Key takeaways: [impact on resilience and observability]
```
...
- Key takeaways: [link test maturity to workflow confidence]
```
...
- Blueprint contributions: [how findings advance the unified AI-driven workflow]
- Priority investigations for next round: [gaps in knowledge about directories]
```
...
### Target 1: Archon-main (`/home/haymayndz/INTEGRATE/Archon-main`)

**Structural & Capability Analysis Summary:**
- Structure: [...]
- Code Quality: [...]
- Documentation: [...]
- Configuration: [...]
- Error Handling: [...]
- Testing: [...]
- Key Takeaways: [How this directory contributes to the unified AI workflow]

**Approved Suggestions Status:**
[List every approved suggestion from the register with status badges, e.g., `- [Session 03 | Status: IMPLEMENTED] Exact suggestion text (file path, priority)`]
...
- [Briefly document newly confirmed wins derived from Implemented Highlights, including file path evidence]
...
### Target 2: context-engineering-intro-main (`/home/haymayndz/INTEGRATE/context-engineering-intro-main`)

[Repeat same structure as Target 1]

### Target 3: mcp-crawl4ai-rag-main (`/home/haymayndz/INTEGRATE/mcp-crawl4ai-rag-main`)

[Repeat same structure as Target 1]

### Target 4: ottomator-agents-main (`/home/haymayndz/INTEGRATE/ottomator-agents-main`)

[Repeat same structure as Target 1]

### Target 5: SuperTemplate-master (`/home/haymayndz/INTEGRATE/SuperTemplate-master`)
...
## AI-Driven Workflow Blueprint (Session Draft)

- **Vision Update**: [How the session findings evolve the intelligent end-to-end workflow]
- **Key Building Blocks**: [Modules/approaches to integrate across directories]
- **Immediate Integration Steps**: [Top 2-3 concrete moves toward the unified workflow]
- **Evidence Map**: [Reference back to specific targets/sections that support the blueprint]
...
- [ ] All five target directories analyzed
...
1. **"KUNG MERON LANG NA GAPS" Principle**: Only add suggestions if genuine gaps exist. Do not add suggestions for completeness if no gaps are found.
